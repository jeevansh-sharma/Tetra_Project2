...
const PortfolioImages = () => {
  ...
  return (
    <section className='flex flex-col max-w-5xl mx-auto '>
      ...
      <h1 className={`${outfit.className} text-7xl font-semibold text-center`}>A Glimpse <span className={`${outfit.className} font-light`}>to Our World</span> </h1>
      <div  className='w-[7rem]   h-[0.3rem] bg-[#F97316] mt-4'></div>
      </div>
      <div className='columns-3 space-y-1 '>
      {images.map((image, index) => (
        <div key={index} className=' rounded-md overflow-hidden py-[7.5px]'>
          <img
            src={image}
